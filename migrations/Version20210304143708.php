 ...
 */
final class Version20210304143708 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__invoice AS SELECT id, supplier_id, recipient FROM invoice');
        $this->addSql('DROP TABLE invoice');
        $this->addSql('CREATE TABLE invoice (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, supplier_id INTEGER NOT NULL, recipient VARCHAR(255) NOT NULL COLLATE BINARY, CONSTRAINT FK_906517442ADD6D8C FOREIGN KEY (supplier_id) REFERENCES supplier (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO invoice (id, supplier_id, recipient) SELECT id, supplier_id, recipient FROM __temp__invoice');
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__invoice AS SELECT id, supplier_id, recipient FROM invoice');
        $this->addSql('DROP TABLE invoice');
        $this->addSql('CREATE TABLE invoice (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, recipient VARCHAR(255) NOT NULL, supplier_id INTEGER DEFAULT NULL)');
        $this->addSql('INSERT INTO invoice (id, supplier_id, recipient) SELECT id, supplier_id, recipient FROM __temp__invoice');
        $this->addSql('DROP TABLE __temp__invoice');
        $this->addSql('CREATE INDEX IDX_906517442ADD6D8C ON invoice (supplier_id)');
        $this->addSql('DROP INDEX IDX_5904BEAD2989F1FD');
